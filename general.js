...
function populateTimeZones() {
    ...
    let counter = 2;
    finalList.push({ tz: zones[0], offset: zones[2].substring(0, zones[2].lastIndexOf(' ')) });
    finalList.push({ tz: zones[2].substring(zones[2].lastIndexOf(' ') + 1), offset: zones[4].substring(0, zones[4].lastIndexOf(' ')) });
    ...
    for (let i = 4; i < (zones.length - 1); i++) {
        ...
        if (item.substring(item.lastIndexOf(' ') + 1) == zones[i - 2].substring(zones[i - 2].lastIndexOf(' ') + 1)) {
            ...
            finalList.push({ tz: item.substring(item.lastIndexOf(' ') + 1) + ' - ' + zones[i + 1], offset: zones[i + 2].substring(0, zones[i + 2].lastIndexOf(' ')) })
        }
        else {
            finalList.push({ tz: item.substring(item.lastIndexOf(' ') + 1), offset: zones[i + 2].substring(0, zones[i + 2].lastIndexOf(' ')) })
        }
...
async function timeZone(message, params, user) {
    ...
    let indexOri = zoneList.find((value, index) => { return value.tz == args[0] });
