...
function populateTimeZones() {
    ...
    for (item of zones) {
        ...
        if (item.includes('UTC')) {
            finalList.push({ tz: timeZone.replace('\t', ' - '), offset: item.substring(item.indexOf("UTC") + 3) })
            timeZone = '';
        }
    }
    return finalList;
...
//Need to take into account offsets with :30, more than one offset, proper time format input checking and formatting at the end;
async function timeZone(message, params, user) {
    ...
    let indexOri = zoneList.find((value, index) => {
        return value.tz == args[0]
    })
    ...
    let indexDes = zoneList.find((value, index) => { return value.tz == args[2] })
    ...
    indexOri.offset = indexOri.offset.charAt(0) == '-' ? Number(indexOri.offset.substring(1)) * -1 : Number(indexOri.offset.substring(1));
    indexDes.offset = indexDes.offset.charAt(0) == '-' ? Number(indexDes.offset.substring(1)) * -1 : Number(indexDes.offset.substring(1));

    console.log("ORI: ", indexOri);
    console.log("DEST: ", indexDes);
    console.log("TIME: ", args[1]);
    console.log(Number(indexOri.offset))
    console.log("NEW TIME: ", Number(args[1]) - (Number(indexOri.offset) - Number(indexDes.offset)))
    message.channel.send("NEW TIME: " + (Number(args[1]) - (Number(indexOri.offset) - Number(indexDes.offset))));
    return 1;
